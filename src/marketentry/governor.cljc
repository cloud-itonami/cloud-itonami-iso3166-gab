(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Gabon-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Gabon-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-ga-entity?
                                       true` (near-universal for Gabon
                                       ...
                                       Grounded in Gabonese legal entity
                                       (RCCM) registration typically required
                                       for public awards (contracting
                                       authorities / ARMP).
...
(defn- ga-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-ga-entity? true`, INDEPENDENTLY verify
  `:has-ga-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-ga-entity?` ground truth (most Gabon public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-ga-entity? e))
                 (not (true? (:has-ga-entity? e))))
        [{:rule :ga-entity-missing
          :detail (str subject " はガボン居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (ga-entity-missing-violations request st)
