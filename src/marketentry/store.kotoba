(ns marketentry.store
  "SSoT for the GAB market-entry compliance actor, behind a `Store`
  ...
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems GA" :portal "ARMP e-procurement"
    ...
    "eng-4" {:id "eng-4" :operator "Higashi Export" :portal "ARMP e-procurement"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-ga-entity? true :has-ga-entity? false
             :requires-rccm? true :rccm-verified? true
             :drafted? false :submitted? false
             :jurisdiction "GAB" :status :intake}
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               requires-ga-entity? has-ga-entity?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-ga-entity?)  (assoc :engagement/requires-ga-entity? requires-ga-entity?)
    (some? has-ga-entity?)       (assoc :engagement/has-ga-entity? has-ga-entity?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-ga-entity? :engagement/has-ga-entity?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-ga-entity? (boolean (:engagement/requires-ga-entity? m))
     :has-ga-entity? (boolean (:engagement/has-ga-entity? m))
...
(defn kotobase-store
  ...
  [{:keys [db-api seq-fn]}]
  (->KotobaseStore (persist/store {:db-api db-api :actor "marketentry-gab"})
